...
from quant_rl_trading.store import config as _config
from quant_rl_trading.store import reader, writer
from quant_rl_trading.store.errors import (
    ConfigNotFound,
    DuplicateIngestRun,
    MissingObservedAt,
    NaiveTimestamp,
...
DEFAULT_CONFIG_FILE = REPO_ROOT / "config" / "quant_rl_trading.yaml"
ROOT_ENV = "QUANT_RL_DATA_ROOT"
...
MEMORY_LIMIT_ENV = "QUANT_RL_DUCKDB_MEMORY_LIMIT"
THREADS_ENV = "QUANT_RL_DUCKDB_THREADS"
...
class Store:
    ...
    def config(self, name: str, *, as_of: datetime) -> Any:
        ...
        """
        # **이름으로 좁히지 않고 표를 통째로 받는다.** 설정은 수백 행이라 읽는
