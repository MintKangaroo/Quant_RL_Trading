...
class TableSpec:
    ...
    #: **참조 속성 테이블** — 게이트를 ``observed_at`` 이 아니라 ``valid_from`` 으로
    #: 건다 (data-contract.md §3 "참조 속성 예외"). 업종 분류처럼 준정적이고
    #: 수익률 예측 정보가 아닌 속성에만 켠다. 이 속성을 2026-08-15 에 처음 받았다는
    #: 사실(observed_at)은 그대로 남기되, 2025 년 시점 조회가 그 종목의 업종을
    #: 못 보는 것은 "미래 훔쳐보기 방지" 가 아니라 "분류를 모르는 척" 이다.
    #: 시세·재무·신호처럼 그 자체가 예측 정보인 테이블에는 절대 켜지 않는다.
