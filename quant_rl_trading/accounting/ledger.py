...
def available_cash(
    ...
    **2026-09-21 정정 — 위 두 문단의 전제가 틀렸다.** 매도대금으로 산 매수도 같은 결제일에 결제되어
    상계된다. 결제일에 돈이 모자랄 수 없고, 그것은 편의가 아니라 결제 구조다. 실측: 증권사 주문가능
    2.1억 · D+2 예수금 1.6억(= 장부 현금)인데 이 함수는 1,671만을 냈다(accounting.md §1).
    함수는 그대로 두고 **설정으로 끈다**(`execution.settlement_days_kr = 0` → 장부 현금이 곧 예산).
    과거 as_of 는 그때의 값(2)으로 재현된다. 레버리지를 막는 것은 이 잠금이 아니라
    "NAV 가 아닌 장부 현금이 예산" 이라는 첫 줄이다.
    """
