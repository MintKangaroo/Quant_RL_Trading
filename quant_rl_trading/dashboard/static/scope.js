...
async function renderFreshness() {
  ...
  const unknown = items.filter((it) => !["ok", "stale", "pending"].includes(it.status));
  ...
  const summary = stale.length
    ...
    : pending.length
    ? `<span class="fresh ok">데이터 ${md(latest)} · 수집 대기 ${pending.map((it) => it.label).join(" · ")}</span>`
  ...
  target.innerHTML = `<span class="fresh-summary">${summary}</span>` + items.map((it) => {
    ...
    if (it.status === "pending") return `<span class="fresh ok">${it.label} ${md(it.observed)} · ${md(it.expected)} 수집 대기</span>`;
