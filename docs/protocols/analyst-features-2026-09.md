...
## 결과 — 시행 B (2026-08-29 측정, `tools/trial_analyst_features.py --trial B --save`)

```
B1 직교화 잔차: IC5 +0.0049 (t +0.89, 271일) · IC20 +0.0100 (t +0.97) · 한계기여 ΔIC -0.0008 (t -0.25) · 상위24 h5 초과 +0.04527 · 순IR +2.10 · 연회전 16.23
  판정: 기각

B2 사건창 20세션: IC5 +0.0412 (t +7.64, 270일) · IC20 +0.0636 (t +8.19) · 한계기여 ΔIC -0.0003 (t -0.10) · 상위24 h5 초과 +0.03074 · 순IR +1.53 · 연회전 17.16
  판정: 기각

research_trials 기록: analyst/B · protocol 1d6684d758646ad9
```

**판정: B1·B2 모두 기각** — 채택 기준(한계기여 ΔIC(h5) NW t ≥ 2)에 미달. event 는 어떻게
잘라도 fundamental 위 **횡단면 정보 증분이 0** 이다(LOO ΔIC −0.0008 / −0.0003).

기록해 둘 부수 관찰(기준이 아니므로 판정에 안 쓴다): B1(직교화 잔차)을 섞으면 상위 24 h5
초과수익 +0.030 → +0.045, 순IR 1.41 → 2.10 이 나왔다. 회전은 5.6 → 16.2 배로 뛴다. 이는
"상위 24 만 보는 지표" 와 "전 종목 IC" 가 다른 것을 재기 때문일 수 있고, 한 표본이다.
쓰려면 **다음 분기에 별도 사전등록**(기준: 상위 24 순IR, 회전 상한 포함)으로 다시 잰다 — 이번
결과를 보고 기준을 바꾸지 않는다.

B2 의 "사건 창" 은 사건 날짜 표가 없어 **피처 벡터의 변화**를 사건으로 읽었다(도구 주석).
프로토콜과의 차이로 적는다.
