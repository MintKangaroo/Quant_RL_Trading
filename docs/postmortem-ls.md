...
> ⚠️ 이 절은 파일 단위 인벤토리다. 각 모듈 내부의 오염 여부(RL 코드 의존), 하드코딩 임계치,
> 이식 난이도는 **아직 검증하지 않았다.** 4단계(Collector) 착수 시 모듈별로 다시 확인할 것.
...
1. **I/O 모듈별 오염 여부** — `broker/*.py` 가 `env/` 나 `agent/` 를 import 하는지. 4단계 착수 전 필수.
2. **`data/schema.py`** — 저장 스키마에 시각 필드가 어떻게 들어 있는지. 5(c) 의 근거 보강.
3. **LS_USA worker 가드 체인** — KR 과 동일한지, 미장 특수사항(서머타임, PDT)이 어디 있는지.
4. **`backtest/walk_forward.py`** — fold 분할에 purge 가 실제로 없는지 확인.
5. **LS_KR / LS_USA `ls_client.py` 차이** (566 vs 439줄) — 공통화 가능한지.
