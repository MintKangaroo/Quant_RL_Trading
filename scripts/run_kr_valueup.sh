#!/usr/bin/env bash
# 시행 BA(docs/protocols/kr-valueup-2026-09.md). 판정이 있으면 건너뛴다. 무거운 작업이 돌면 다음 회차.
set -u
cd /home/mintkangaroo/Project/Quant_RL_Trading || exit 1
LOG=logs/trial-kr-valueup-BA.log
...
{
    echo "=== $(date '+%F %T') 시행 BA ==="
    MALLOC_ARENA_MAX=2 QUANT_RL_DUCKDB_MEMORY_LIMIT=1000MB nice -n 5 .venv/bin/python -u tools/trial_kr_valueup.py --save
    echo "rc=$?"
} >> "${LOG}" 2>&1
