...
ADOPTED=""
{
  ...
  for g in G1 G2 G3 G4 G5 G6 G7 G8; do  # G8 = 국장 잠정실적(2026-09-23 추가 등록)
    # G8 은 공시 원문 완비(9/27 경)와 9/28 04:37 추가 적재 뒤에 잰다(G8 등록 정정 3). 휴장 당김(9/24)으로 사슬이 그 전에 돌 수 있다.
    if [ "${g}" = "G8" ] && [ "$(date +%Y%m%d)" -lt 20260928 ]; then
      echo "G8 는 9/28 잠정실적 추가 적재 뒤에 잰다 — 이번 회차는 여기서 멈춘다"
      break
    fi
