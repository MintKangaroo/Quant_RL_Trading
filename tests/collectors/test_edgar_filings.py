...
def test_424b2_is_not_dilution() -> None:
    """**424B2 는 희석이 아니다** — 선반등록 하의 정형 노트 발행이다.

    실측 2026-08-20: 미장 dilution 135,987행 중 424B2 가 125,246행(92%)이고,
    그 97%가 12종목(JPM-PM·MS-PQ·GS-PD·C-PR…)에 몰려 있다. 전부 은행
    우선주·ETN 이고 9/12 는 매매 대상도 아니다.

    매매 대상에 떨어지는 비율이 이것만 유별나다 — 424B2 19.3% 대 424B5
    82.4%, 나머지 doc_type 79.7%. 424B5(선반 인출 = 실제 증자)는 정상이다.
    **성적이 나쁜 게 아니라 애초에 다른 사건이다.**
    """
    assert classify("424B2", ()) == "other"
    # 진짜 증자는 그대로 희석이다. 424B2 하나만 뺀 것이지 424B 계열을
    # 통째로 버린 것이 아니다.
    for form in ("424B1", "424B3", "424B4", "424B5"):
        assert classify(form, ()) == "dilution", form
