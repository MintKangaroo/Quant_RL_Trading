...
1. **파티션 압축 집행.** 조회 비용이 행이 아니라 **파일 수**로 붙는다. `indices` 복사본 실측:
   파일 8,523 → 1,733 · 39.9MB → 10.2MB · 조회 526ms → 118ms, 결과는 md5까지 동일.
   도구·시험 준비 완료(`quant_rl_trading/store/compaction.py`). **장 중·수집 중에는 안 돌린다** —
   원본을 지우는 순간이 읽는 쪽과의 유일한 경합 지점이다.
2. **노출 축 후속 시행.** 시행 V가 "국면 축만" 을 채택했지만, 탈락한 셋은 최악 20세션 블록에서
   **1.1bp** 차이로 갈렸다 — 잡음이다. 노출 장치 전체가 위험을 거의 못 바꾸고(MDD 폭 0.06%p)
   수익만 깎는다는 사실이 남는다. 더 센 검정으로 다시 물어야 한다(다음 분기).
3. **백테스트 저널 재사용.** 설정을 바꾸고 상주 샌드박스에 다시 돌리면 노출 기준점을 앞 실행에서
   읽는다(`EventLog.flush` 가 같은 run_id 를 건너뛴다). 지금은 "깨끗한 샌드박스" 가 처방이고,
   근본 수정(루프가 직전 배수를 넘기고 첫 세션만 저널을 본다)은 미착수. 라이브는 무관하다.
4. **미장 편입.** LS 모의투자가 해외주식을 막아(rsp_cd 01900) shadow 장부로만 돈다. 실계좌·보류
   중 선택이 미결이다.
5. **mypy·ruff 정리.** 계속 미뤄지고 있다.
