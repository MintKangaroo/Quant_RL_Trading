...
사용자 지적에서 시작했다: "K200 상승의 대부분은 삼성·하이닉스이고 그건 최근 1년 안에 일어난 일이다."
실측이 그대로였다 — K200 연도별 수익률 2021 −1.4% · 2022 −26.4% · 2023 +23.5% · 2024 −11.9% ·
**2025 +90.7% · 2026 상반 +119.6%**. 그동안의 모든 시행(P·U·V·Y…)은 그 폭등 구간 **안에서만** 판정됐다.

원천(시세·재무·수급·공시·명단)은 **전부 2021-08 까지 있었다.** 없던 것은 그 구간의 점수 이력뿐이었다.
`tools/bake_long_panel.py` 로 창고 `signals` 를 읽어 **확장 패널**을 만들었다(재채점이 아니라 읽기라 28분):

| 산출물 | 규모 |
|---|---|
| Analyst 점수 8종 | 4,470만 행 (2021-11~2026-06, 1,141세션) |
| 타깃 h5 · h20 · h60 | 각 310만 행 |
| 매매 유니버스(시점별 판정) | 307만 행 |

이제 시행은 **박스 구간(2022-07~2024-12)과 급등 구간(2025-01~2026-06)을 갈라서** 판정한다. 등록된 채택
기준의 1번이 "두 국면 **모두**에서 지지 않을 것" 이다 — 한 국면에서만 이기는 변형을 거르기 위해서다.

새로 등록한 시행 셋:

| 시행 | 묻는 것 | 문서 |
|---|---|---|
| **Z** | 대형주를 들어야 지수를 이기나 (K200 유니버스 · β 목표 가중 · **시총 상위 2종목 강제 편입** · 지수 대조군) | `docs/protocols/beta-target-megacap-2026-09.md` |
| **AA** | 랭커의 저β 편향이 타깃에서 오나 (h5 수익 대신 **β 잔차·시장 초과**) | `docs/protocols/ranker-target-beta-2026-09.md` |
| **AB** | 5일 지평이 복리 승자를 못 보나 (**지평 앙상블** h5/h20/h60 순위 평균) | `docs/protocols/ranker-ensemble-horizons-2026-09.md` |

AB 가 먼저다. 근거는 같은 날 실측이다 — 랭커는 삼성전자를 **백분위 83.7**(상위 16%)로 보는데 상위 24 는
**상위 0.85%** 라 못 든다. 그리고 두 종목의 h5 선행수익 백분위는 67·68 로 평범한데 누적은 +505%·+1,366%
(전 종목 상위 1.4%·0.3%)다. **"거의 지지 않는 것" 이 복리로 쌓인 것**이고, h5 타깃에는 그것을 볼 자리가 없다.

기각된 것도 남긴다: **시행 Y**(K200 안에서 고르기)는 기각됐다. K200 구성종목만 써도 포트 β 가 0.25~0.29 였고,
상승일 추종 − 하락일 추종(비대칭)이 271세션 내내 음수였다 — **방어처럼 보이던 것은 덜 들고 덜 움직인 결과**다.

### 2026-09-20 운영 수정 셋 (사용자 승인)

| | 무엇 | 왜 |
|---|---|---|
| ① | `execution.settlement_days_us=1`·`_kr=2` | 국장 D+2 를 미장에도 써서 9/15 매도대금 \$351k 가 하루 더 묶였다. 미국은 2024-05 부터 T+1 |
| ② | 워밍업 재생은 주문을 **기록하지 않는다**(`record_warmup=False`) | 9/19: 게이트로 주문 0 이던 전날을 재결정해 \$341k 를 사후에 장부에 박았다 — 운영 사고가 성과에서 지워진다 |
| ③ | 미장 수집 결손 방지 — 재부팅 복구 재기동 + `wait_us_prices.sh` | 수집이 5,600/6,602 에서 끊긴 채 파이프라인이 낡은 시세로 돌았다 |


### 지금 어디 있나

모의계좌는 2026-08-28에 시작해 실주문을 내고 있다. 아래는 **등록된 종료 기준**(KODEX200 대비)으로
`tools/verify_exit_criterion.py` 가 낸 중간 집계다. 판정이 아니라 현재 위치다.
...
초과수익은 음수이고 낙폭은 개선이다. 중단은 **둘 다** 참일 때만이므로 지금 판정하면 계속이다.
참고 수치(관문 아님): 베타 0.20 · 베타 보정 알파 −2.13%p.

`tools/attribute_returns.py` 가 그 차이를 항목으로 가른다(근사가 아니라 항등식, 잔차 0):

| 항 | %p | 읽는 법 |
|---|---|---|
| 선택 | **−3.67** | 고른 24종목이 **살 수 있었던 세계**(동일가중)에 졌다 |
| 유니버스 | +3.41 | 그 세계가 시총가중 ETF를 이겼다 — 우리가 못 고르는 부분 |
| 노출 | −3.03 | 덜 투자해서 손해. 이 구간 시장이 올랐다 |
| 야간갭 | +0.81 | 우리가 한 일이 아니다 |
| 집행 | +0.97 | **실력으로 읽지 말 것** — 지정가라 유리한 값에만 체결된다 |
| 명시비용 | −0.23 | |

**제일 아픈 줄은 선택이다.** 표본 12구간이라 방향만 읽을 수 있지만, "병목은 알고리즘이 아니라
정보" 라는 기존 진단과 같은 방향이다.

**집행 탓이 아닌지 확인했다.** 지정가라 못 산 종목이 있는데(주문 263건 중 43건 미체결), 못 산 종목은
그 뒤 오히려 더 떨어졌다(매수 3일 −2.26% 대 체결 −0.61%). 선택 항은 **무엇을 고르느냐**의 문제다 —
그래서 다음 레버는 전부 정보 쪽(6차 G1~G6 · 시행 W 원피처 · 시행 X 원문 임베딩)이다.

### 날짜가 잡힌 것

| 날짜 | 할 일 | 상태 |
|---|---|---|
| 매 토 14:00 | 주간 IC 측정 — 랭커 감쇠 경보 | 크론 가동 |
| 매 토 | 재료 후보 대장 갱신·추천 | 상시 업무 |
| 매 일 08:00 | 수익 분해 기록 — 판정일에 시계열이 필요하다 | 크론 가동 |
| 화~토 09:10 | 판정 대조군(KODEX200)·기초지수 수집 | 크론 가동 |
| **2026-10-01 이후** | **6차 재료 측정 G1~G7** — 선택 항을 고칠 등록된 경로 (G7 = 미장 내부자 Form 4, 9/19 추가) | 체인·크론 준비 완료, 첫 실행 **10-04(일) 11:00**. 일곱 묶음 × 1~2시간이라 **평일 02:30 에도 돈다**(이미 잰 묶음은 건너뜀) |
| 확장 패널 준비되는 대로 | **시행 AB → Z → AA** — 두 국면에서 판정 | AB 도구 `tools/trial_ranker_ensemble.py` 배선 확인 완료 |
| 2026-09-20 (일) 09:30 | 원피처 캐시 굽기(두 시장 같은 창) — 시행 W 선행 | 압축 뒤 한 번, 크론 걸림 |
| **2026-10-11 (일) 13:00** | **시행 W — 원피처 랭커**(점수 6개 대신 원피처 33개) | 사전등록 완료 `docs/protocols/raw-feature-ranker-2026-09.md` |
| 원문 완비(10월 중순) 뒤 | **시행 X — 공시 원문 임베딩** | 초안 등록. **캐시된 finbert 는 영어 전용**이라 한국어 모델을 먼저 고른다 |
| 2026-11 하순 | 홀드아웃 개봉 | |
| 2026-12-05 | 랭커 재학습 첫 시도 (개봉 뒤 첫 토요일) | |
| **2026-11-25** | **종료 판정 — 60거래일** | 데이터·도구 준비 완료 |

종료 판정은 결과를 보고 기준을 바꾸지 않는다. 계산은 `tools/verify_exit_criterion.py` 하나가
하고, 무엇을 볼지 고르는 자리가 없다. 자세한 것은 [`docs/milestones.md`](docs/milestones.md)
"종료 기준" 절에 있다.

### 남은 일 — 날짜를 잡았다 (2026-09-18)

| 날짜 | 할 일 | 왜 그날인가 |
|---|---|---|
| **매주 일 09:00** (첫 실행 09-20) | **파티션 압축** | 일요일은 08:00 수익분해·11:00 6차측정 말고 비어 있다. **원본을 지우는 순간이 읽는 쪽과의 유일한 경합 지점**이라 장 중·수집 중에는 못 돌린다. 새 파일이 매일 쌓이므로 일회성이 아니라 주간 유지보수다 |
| **2026-09-26 (토) 오전** | **린트 1단계** — `ruff check` 출력을 다시 신호로 | 토 14:00 주간 IC 전에 끝난다. **버그를 고치는 일이 아니다** — 전수 확인 결과 진짜 버그는 0개였다(아래 "린트 실태") |
| **2026-09-27 (일)** | **백테스트 저널 재사용 근본 수정** | 라이브 경로(`daily.run` 시그니처)를 건드리므로 장 없는 날. 그 주 일요일은 압축(09:00)뿐이라 오후가 비어 있다 |
| **2026-10-10 (토) 오전** | **린트 2단계** — mypy 162개 (고치거나 래칫) | 1단계로 소음을 걷은 뒤. **수익을 바꾸지 않으므로 언제든 밀 수 있다** |
| **2026-11-26 (판정 다음 날)** | **미장 편입 결정** — 실계좌 / shadow 유지 / 보류 | 자본 결정이라 60거래일 판정 결과가 있어야 정할 수 있다. LS 모의투자는 해외주식을 막는다(rsp_cd 01900) |
| **2026-11-28 (토)** | **노출 축 후속 시행** | **지금은 창을 못 늘린다** — 홀드아웃(11월 하순 개봉) 전에는 안 본 데이터가 없다. 개봉된 홀드아웃이 곧 "더 센 검정" 이다. 시행 V가 1.1bp로 승자를 가른 것을 여기서 다시 묻는다 |

#### 파티션 압축 — 규모

창고 전체에서 **파일 126,284개 · 3.4GB** 가 파일 여럿짜리 파티션에 들어 있다. 파일/파티션이 가장 나쁜 곳:

| 표 | 파티션 | 파일 | 파일/파티션 |
|---|---|---|---|
| `fundamentals` | 1,312 | 42,583 | 32.5 |
| `insider_trades` | 485 | 17,530 | 36.1 |
| `prices` | 1,302 | 14,186 | 10.9 |
| `signals` | 1,242 | 10,293 | 8.3 |
| `ingest_latency` | 28 | 7,387 | **263.8** |
| `agent_cache` | 88 | 8,349 | **94.9** |

`indices` 복사본 실측(8,523 → 1,733 파일)에서 조회가 **526ms → 118ms**, 용량 39.9MB → 10.2MB,
결과는 md5까지 동일했다. 세션·IC 측정·백테스트·화면이 전부 같은 창고를 읽으므로 이득이 한 곳에
머물지 않는다.

#### 린트 실태 — 1,708개 중 **진짜 버그는 0개** (2026-09-18 전수 확인)

`ruff` 1,708개 중 **1,180개가 `tools/`** 의 줄길이(E501 907)와 세미콜론(E702 376)이다. 연구
스크립트의 문체이지 결함이 아니다. 패키지 안 `RUF002`·`RUF003` 77개는 한국어 문서의 `·` `—` 를
"모호한 유니코드" 로 잡은 **오탐**이다 — 코드가 아니라 설정으로 풀 일이다.

**진짜 버그 부류 열둘은 하나하나 읽어 확인했고, 전부 거짓 양성이었다:**

- `B905`(zip 이 조용히 짧은 쪽에 맞춘다) 5곳 — 두 계열이 **같은 프레임에서 나와** 길이가 같다.
  `event.py` 는 같은 `groupby` 조각의 두 열이고, `trading.py` 셋은 `navs` 와 그로부터 만든
  `cummax`·`daily`·`sessions` 다. `strict=True` 는 고침이 아니라 **주장의 명문화**일 뿐이다.
- `B023`(반복변수 포획) 7곳 — 전부 `lambda` 를 **같은 반복 안에서 즉시 소비**한다. 늦은 바인딩이
  문제가 되려면 람다가 반복을 넘겨 살아야 하는데 그런 자리가 없다.

**그래서 "고쳐야 하는 것" 은 없다.** 문제는 개수가 아니라 **`ruff check` 출력이 아무도 안 읽는
벽이라는 것**이다. 진짜 결함이 생기면 1,709번째로 묻힌다. 1단계의 목표는 0을 만드는 것이 아니라
**출력을 다시 신호로 만드는 것**이다: 체계적 오탐을 설정으로 끄고(한국어 confusables, `tools/` 의
줄길이·세미콜론), 자동 수정되는 것(F401 8 · I001 14 · RUF100 7)을 정리해 `ruff check` 가 0 을
내게 한 뒤 CI 게이트로 건다.

`mypy` 162개도 같은 성질이다 — 패키지가 strict 를 선언했는데 162개가 떠 있으면 strict 는 어디서도
강제되지 않는다. 고치거나, "새 오류를 늘리지 않는다" 는 래칫으로 바꾸거나 둘 중 하나다.
**둘 다 11-25 판정 전에 수익을 바꾸지 않는다** — 그래서 2단계는 언제든 미룰 수 있는 자리에 둔다.

### 기록에 남긴 교훈 (2026-09-18)

- **사전등록 게이트는 `≥` 가 아니라 "의미 있는 여유" 로 쓴다.** 시행 V의 네 번째 기준이 1.1bp로
  승자를 갈랐다. 동률·1bp를 판정에 쓰면 사전등록이 잡음을 제도화한다.
- **판정 도구와 생산 코드가 같은 경계를 쓰면 사이좋게 틀린다.** 데드밴드의 부동소수 경계
  (`1.0 - 0.8 = 0.19999999999999996`)가 둘 다에 있어 없는 우위를 만들어 냈다.
- **메모리가 빠듯한 기계에서 캐시는 크기가 아니라 단가로 고른다.** 큰 프레임을 담았더니 스왑으로
  밀려 첫 응답이 4.9초 → 10~14초로 뒤집혔다.
- **rc 를 파이프에 넘기지 않는다.** `pytest | tail` 은 실패를 삼킨다. 이 저장소가 이미 겪은 함정에
  다시 빠졌다.
...
기존·신규 선택 테스트 **409개 통과**, 추가 경계 재검사 **21개 통과**를 확인했다(중복 포함, 합산 금지). 독립 `execution-safety` CI가 주문·브로커 회귀 테스트를 실행한다. 계좌 전체 예약 현금·포트폴리오 한도는 아래 후속 패치에 반영했다. 장중 조치의 영속 claim과 KR 확인 이벤트는 아래 후속 패치에 반영했다. 실제 브로커 주문·운영 배포·모델 재학습은 실행하지 않았다.
...
세 번째 패치는 포트폴리오 최종 제약을 재검사한다. 충족할 수 없는 RC·베타 상한, 결측 위험 입력, 잘못된 공분산을 거절하고 세션에 차단 사유를 남긴다. 제약·배분·세션 선택 테스트 **31개 통과**를 확인했다. score fallback의 위험 인증은 별도 후속 범위이며, 계좌 전체 예약 예산은 아래 후속 패치에 반영했다.

최종 불변식·포트폴리오·allocator·세션 검사 **266개 통과**, RL 패키지가 없는 격리 환경에서도 불변식 **112개 통과**를 확인했다. 검증 명령과 실행별 범위는 follow-up 문서에 기록했다.

### 개선 진행 — 계좌 예약·현금·노출 한도

공통 `risk` 계층이 체결 장부와 미체결 주문을 함께 검사한다. 같은 계좌의 다른 주문이 현금을 중복 사용하지 못하도록 KRW/USD별 수수료 포함 예약, 실제 보유량을 넘는 매도 차단, 종목·총노출·동시 보유 수·일일 손실·MDD 한도를 적용했다. 분할 주문 전체를 승인한 뒤 첫 조각을 전송하며, 재호가도 현재 예산을 다시 검사한다.

부분체결 후에는 잔량만 예약한다. UNKNOWN을 날짜 경과로 만료 처리하지 않으며, 과거 주문번호를 당일 응답에 연결해 체결을 만들지 않는다. 미전송 조각은 명시적 마감 처리로 철회할 수 있다. 백테스트는 PaperBroker용으로 승인된 조각만 체결하고 D+1 시도 후 예약을 해제한다. 실제 브로커용 미전송 예약은 모의체결 대상이 아니다. 미국 매도대금의 결제일은 시장 현지 날짜로 계산한다.

`config_version=2`에는 `risk.max_gross_exposure=1.0`, `risk.max_positions=30`, `risk.max_daily_loss=0.03`을 추가했다. 종목 상한과 MDD 상한은 기존 allocator/kill 설정을 공유한다. 이 값들은 수익성 검증으로 선택한 최적값이 아니다. 기존 창고의 설정은 변경하지 않았으며, 적용 시 발효시각을 기록해야 한다. 필요한 설정·계좌 평가가 없으면 신규매수를 차단한다.

잠금 범위는 **같은 POSIX Store 루트의 계좌 한 개**다. 다른 저장소나 호스트가 같은 브로커 계좌를 공유하는 경우까지 보장하지 않는다. 주문일 지정 과거 대사와 누락된 확인 이벤트 복구는 후속 작업이다. KR 확인 이벤트와 조치별 영속 claim은 아래에 반영했다. 핵심 회귀 테스트 **325개 통과**, 별도 설정·불변식 검사 **128개**, 최신 백테스트·shadow 재생 검사 **12개 통과**를 확인했다(중복 포함). 상세 계약과 실행별 검증 결과는 [postmortem follow-up](docs/postmortem-follow-up-2026-09-09.md)에 기록한다.

### 개선 진행 — 취소·정정 확인과 부분체결 대사

취소·정정을 보내기 전에 `execution_events`에 intent를 기록한다. 프로세스 중단·재시작 뒤에도 같은 조치를 재전송하지 않으며, 재시도 횟수·타이머·원 기준가·확인된 정정 주문번호를 복원한다. **API 접수 ACK만으로 취소 완료나 지정가 변경을 확정하지 않는다.**

KR 확인 수신 도구 `tools/watch_order_events.py`를 추가했다. 인증된 SC2/SC3를 계좌 지문·주문일·종목·방향·원주문번호·조치 기록과 대조한다. 확인된 취소 수량만 예약에서 빼며, 부분 취소 뒤 미기록 체결분은 계속 예약한다. 중복 확인은 한 번만 반영하고 상충된 확인은 거절한다. 체결 대사도 새 전송 기록의 계좌·주문일을 검사한다.

부분체결의 누적 평균가가 바뀌면 **누적 거래대금 − 이미 기록한 거래대금**으로 새 체결분을 계산한다. 원수량 초과·수량 역행·비정수 체결을 장부에 적지 않는다. 수수료·세금은 현재 설정 모델에 따른 추정이며, 증권사 실제 비용 명세와의 대사는 별도다.

최종 회귀 검사 **429개 통과**(실데이터 의존 3개 제외), 별도 overlay·shadow 재생 **8개 통과**를 확인했다. 실행별 테스트는 중복될 수 있다.

수신 도구는 주문 추격 전에 별도로 실행해야 한다. 현재 KR 경로만 구현했으며, 실제 브로커 접속·운영 등록은 실행하지 않았다. 연결 끊김·이벤트 누락·부분 정정·주문일/계좌 지문 없는 기존 주문·US 최종 취소는 자동 해결하지 않는다. 이 경우 예약을 유지하고 날짜 지정 대사가 필요하다. 상세 회귀 검증과 남은 작업은 [follow-up](docs/postmortem-follow-up-2026-09-09.md#취소정정-확인과-부분체결-대사-후속-패치)에 기록한다.
...
| 09:19~15:19 (재호가 1분 전) | 집행 | 취소·정정 확인 수신 170초 (`tools/watch_order_events.py`) — 읽기 전용, 확인이 와야 예약이 당일에 풀린다 |
...
| 토 14:00 | 주간 IC 측정 KR·US `--save` → 학습 탭 랭커 감쇠 경보 (`scripts/measure_ic_weekly.sh`, M5 ①) |
...
| [`docs/protocols/exposure-timing-2026-09.md`](docs/protocols/exposure-timing-2026-09.md) | 시행 U — 노출 데드밴드. 부동소수 결함과 재측정 기록 |
