...
기존·신규 선택 테스트 **409개 통과**, 추가 경계 재검사 **21개 통과**를 확인했다(중복 포함, 합산 금지). 독립 `execution-safety` CI가 주문·브로커 회귀 테스트를 실행한다. 계좌 전체 예약 현금·포트폴리오 한도는 아래 후속 패치에 반영했다. 장중 조치의 영속 claim 및 최종 취소 확인은 후속 작업이다. 실제 브로커 주문·운영 배포·모델 재학습은 실행하지 않았다.
...
잠금 범위는 **같은 POSIX Store 루트의 계좌 한 개**다. 다른 저장소나 호스트가 같은 브로커 계좌를 공유하는 경우까지 보장하지 않는다. 최종 취소 증거·주문일 지정 과거 대사·조치별 영속 claim은 후속 작업이다. 핵심 회귀 테스트 **325개 통과**, 별도 설정·불변식 검사 **128개**, 최신 백테스트·shadow 재생 검사 **12개 통과**를 확인했다(중복 포함). 상세 계약과 실행별 검증 결과는 [postmortem follow-up](docs/postmortem-follow-up-2026-09-09.md)에 기록한다.
