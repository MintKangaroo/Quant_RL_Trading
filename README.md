...
| **2026-09-26 (토) 오전** | **린트 1단계** — 진짜 결함 부류와 규칙 오탐 정리 | 토 14:00 주간 IC 전에 끝난다. 아래 "린트 실태" 참고 |
| **2026-09-27 (일)** | **백테스트 저널 재사용 근본 수정** | 라이브 경로(`daily.run` 시그니처)를 건드리므로 장 없는 날. 그 주 일요일은 압축(09:00)뿐이라 오후가 비어 있다 |
| **2026-10-10 (토) 오전** | **린트 2단계** — mypy 162개 | 1단계로 소음을 걷은 뒤. 10-04 6차 측정과 겹치지 않는다 |
...
#### 린트 실태 — 1,708개가 다 같은 무게가 아니다

`ruff` 1,708개 중 **1,180개가 `tools/`** 의 줄길이(E501 907)와 세미콜론(E702 376)이다. 연구
스크립트의 문체이지 결함이 아니다. 패키지(`quant_rl_trading/`) 안에서도 `RUF002`·`RUF003`
77개는 한국어 문서의 `·` `—` 를 "모호한 유니코드" 로 잡은 **오탐**이다 — 코드가 아니라 설정으로
풀 일이다.

진짜로 고칠 것은 적다: `B905`(zip strict 없음) 5 · `F401`(미사용 import) 8 · `I001` 14 ·
`RUF100`(죽은 noqa) 7 · `RUF005` 4, 그리고 `tools/`·`tests/` 의 `B023`(반복변수 포획) 7개.
**`B023` 은 패키지 안에 없다** — 확인했다. `mypy` 162개는 패키지 전용이라 따로 2단계로 둔다.
